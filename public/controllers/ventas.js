angular.module('MyApp')
    .factory('Informacion', function($http) {
        var fInfo = {};
        fInfo.info = {};
        fInfo.getDni = function(dni) {
            return $http.get('Dni/' + dni, dni).success(function(data) {
                return data
                console.log(data);
            })
        }
        fInfo.getRuc = function(ruc) {
            return $http.get('Ruc/' + ruc, ruc).success(function(data) {
                return data
                console.log(data);
            })
        }
    ...
    .factory('fVentas', function($http) {
        var fVenta = {};
        fVenta.ventas = [];
        fVenta.venta = {};

        fVenta.getAll = function() {
            return $http.get('/ventas')
                .success(function(data) {
                    angular.copy(data, fVenta.ventas)
                    return fVenta.ventas
                    console.log(fVenta.ventas);
                })
        }
        fVenta.add = function(venta) {
            return $http.post('/venta', venta)
                .success(function(venta) {
                    fVenta.ventas.push(venta);
                })
        }
        fVenta.update = function(venta) {
            return $http.put('venta/' + venta._id, venta)
                .success(function(data) {
                    var indice = fVenta.ventas.indexOf(venta);
                    fVenta.ventas[indice] = data;
                })
        }
        fVenta.delete = function(venta) {
            return $http.delete('/venta/' + venta._id)
                .success(function() {
                    var indice = fVenta.ventas.indexOf(venta);
                    fVenta.ventas.splice(indice, 1);
                })
        }
        return fVenta;
    })
    .controller('CtrlVentas', function($scope,$window, $http, $state, fStockProductos, fVentas,tipocomun, fClientes, Account, fProveedores, fSucursales, fProductos, toastr) {
        $scope.encabezado = "";
        $scope.encabProd = "";
        $scope.variosproductos = [];
        $scope.venta1 = { CodigoBarra: String, Nombre: String, Cantidad: Number, UMedida: String, PrecioUnitario: Number, PrecioTotal: Number }
        ...
        $scope.addinicio = function() {
            toastr.clear();
            toastr.success('Ingrese el nro de dni del cliente, el sistema completara los campos, sea paciente gracias!');
        ...
        $scope.addVenta = function() {
            if($scope.venta.NombreCliente==null || $scope.venta.NombreSucursal==null){
                toastr.clear();
                toastr.warning('complete los campos porfavor, sino no podra emitir la venta, Muchas gracias');
            }else{
                $scope.venta.TipoPago = parseInt($scope.tipo_pago.selectedOption.id);
                $scope.venta.Estado = parseInt($scope.estado_venta.selectedOption.id);
                ...
                console.log($scope.venta);
                fVentas.add($scope.venta).then(function(response){
        ...
        $scope.listStockProducto = function() {
            ...
            fVentas.getAll()
                .then(function(response) {
                    ...
                    $scope.totalItems1=$scope.ventas.length;
        ...
        $scope.agregar = function() {
            toastr.clear();
            toastr.success("para emitir la venta complete los campos porfavor");
            $scope.venta = {};
            $scope.variosproductos = [];
        ...
        $scope.tuproducto2 = function(producto) {
            ...
            //var datos=$scope.stockproductos;
        ...
        $scope.tuproducto3 = function(producto) {
            $scope.seleccionar = false;
            $scope.caracteristicaa = false;
            console.log(producto);
            //var longitud = fProductos.productos.length;
            //console.log(longitud);
            var datos=$scope.stockproductos;
            console.log(fProductos.productos);
            for (var index = 0; index < datos.length; index++) {
                //var element = array[index];
                if (datos[index]._id == producto) {
                    console.log(datos[index]);
                    $scope.producto = datos[index];
                    for(var index1=0;index1<fProductos.productos.length;index1++){
                      if(fProductos.productos[index1]._id==datos[index].Producto){
                        $scope.Descripciones = fProductos.productos[index1].Descripcion;
                      }
                    }
